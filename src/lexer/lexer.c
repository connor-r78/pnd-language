...
const char* tokenNames[] = {"TOKEN_LPAREN", "TOKEN_RPAREN", "TOKEN_SYMBOL",
...
Token* next_token() {
  ...
  size_t i = 0;
  while ((isalnum(*current_input_ptr) ||
          is_allowed_special_character(*current_input_ptr)) &&
         i < MAX_SYM_SIZE - 1 && (current_input_ptr - input_start_ptr < len)) {
    buffer[i++] = *current_input_ptr++;
    current_column++;
  }
  if (i >= MAX_SYM_SIZE - 1) {
